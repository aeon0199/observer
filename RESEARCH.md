...
- **Next recommended action**: TD2 applied ✓, E2 phase B complete ✓ (see F11/F12). H6 partially rejected; late-layer isn't a universal stability win. Two legitimate paths forward:
  - **(a) E2 phase C — magnitude sweep at L27:** [0.5, 0.7, 1.0] × 5 seeds × 2 prompts = 30 runs. Pin the magnitude at which DSR crosses 2 (if it ever does). Tests whether seed-dominated regime is escapable by brute-force magnitude or whether it's a fundamental property of noise-mode perturbation.
  - **(b) E3 — length-controlled baselines.** Ignore hysteresis for now; go make sure the divergence signal itself behaves reasonably across prompts and lengths before building more claims on top.
  - Recommend **(a)** next — it directly tests whether the "signal-dominated regime" GPT predicted actually exists at any magnitude, or whether noise-mode perturbation is fundamentally seed-dependent.
